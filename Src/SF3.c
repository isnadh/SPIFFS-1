...
void writePage(uint8_t* writeData, int address) {
	...
	sndBytes[1] = (uint8_t)(address >> 16);
	sndBytes[2] = (uint8_t)(address >> 8);
	sndBytes[3] = (uint8_t)address;
...
void my_spi_read(int addr, int size, u8_t *buf){

}

void my_spi_write(int addr, int size, u8_t *buf){
